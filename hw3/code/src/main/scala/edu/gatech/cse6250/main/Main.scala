...
object Main {
  ...
  def testClustering(phenotypeLabel: RDD[(String, Int)], rawFeatures: RDD[(String, Vector)]): (Double, Double, Double) = {
     ...
     */
    val kmean_n = new KMeans().setSeed(6250L).setK(3).setMaxIterations(20).run(featureVectors).predict(featureVectors)
    val kmean_pred = features.map(_._1).zip(kmean_n).join(phenotypeLabel).map(_._2)
    val kMeansPurity = Metrics.purity(kmean_pred)
    ...
    val streamingKmeans_pred_grouping = features.map(_._1).zip(streamingKmeans_pred).join(phenotypeLabel).map(_._2)
    val streamKmeansPurity = Metrics.purity(streamingKmeans_pred_grouping)
  ...
  def loadRddRawData(spark: SparkSession): (RDD[Medication], RDD[LabResult], RDD[Diagnostic]) = {
    ...
    /*LabResult*/
    val LabResult_raw = CSVHelper.loadCSVAsTable(spark, "data/lab_results_INPUT.csv", "lab_results_raw")
    val df_LabResult = spark.sql("SELECT Member_ID as patientID, Date_Resulted, Result_Name, Numeric_Result FROM lab_results_raw WHERE Numeric_Result != ''")
    val labResult: RDD[LabResult] = df_LabResult.map(f => LabResult(f(0).toString, sqlDateParser(f(1).toString), f(2).toString.toLowerCase, f(3).toString.filterNot(",".toSet).toDouble)).rdd
    /*Medication*/
    val Medication_raw = CSVHelper.loadCSVAsTable(spark, "data/medication_orders_INPUT.csv", "medication_orders_raw")
    val df_Medication = spark.sql("SELECT Member_ID as patientID, Order_Date, Drug_Name FROM medication_orders_raw")
    val medication: RDD[Medication] = df_Medication.map(f => Medication(f(0).toString, sqlDateParser(f(1).toString), f(2).toString.toLowerCase)).rdd
    /*Diagnostic*/
    val Diagnostic_raw = CSVHelper.loadCSVAsTable(spark, "data/encounter_INPUT.csv", "encounter_raw")
    val Dx_raw = CSVHelper.loadCSVAsTable(spark, "data/encounter_dx_INPUT.csv", "Dx_raw")
    val df_Diagnostic_dx = spark.sql("SELECT encounter_raw.Member_ID as patientID, encounter_raw.Encounter_DateTime, Dx_raw.code FROM encounter_raw JOIN Dx_raw ON encounter_raw.Encounter_ID = Dx_raw.Encounter_ID")
    val diagnostic: RDD[Diagnostic] = df_Diagnostic_dx.map(f => Diagnostic(f(0).toString, sqlDateParser(f(1).toString), f(2).toString)).rdd
