...
class MyVariableRNN(nn.Module):
	def __init__(self, dim_input):
		...
		self.layer1 = nn.Sequential(nn.Dropout(p=0.5),nn.Linear(dim_input, 128, bias=False),nn.Dropout(p=0.5))

		self.rnn1 = nn.LSTM(input_size=128, hidden_size=128, num_layers=1, batch_first=True)   # bidirectional
		self.rnnl1 = nn.Linear(in_features=128, out_features=1)
		self.rnnl1.bias.data.zero_()

		self.rnn2 = nn.LSTM(input_size=128, hidden_size=128, num_layers=2, batch_first=True)
		self.rnnl2 = nn.Linear(in_features=128, out_features=128)
		self.rnnl2.bias.data.zero_()

		self.rnn3 = nn.GRU(input_size=128, hidden_size=128, num_layers=1, batch_first=True)
		self.rnnl3 = nn.Linear(in_features=128, out_features=128)
		self.rnnl3.bias.data.zero_()
	...
	def forward(self, input_tuple):
		...
		out = torch.tanh(self.rnnl2(self.rnnl3(gps)))
