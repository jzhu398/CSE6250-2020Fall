...
object PowerIterationClustering {
  ...
  def runPIC(similarities: RDD[(Long, Long, Double)]): RDD[(Long, Int)] = {
    ...
    val setting_PIC = new PIC().setK(3).setMaxIterations(100).setInitializationMode("degree")
    val assign_PIC = setting_PIC.run(similarities)
    val clusteringResult = assign_PIC.assignments.map(f => (f.id, f.cluster))
