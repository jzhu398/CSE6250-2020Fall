...
-- ***** your code below *****
FROM (SELECT COUNT(alive_events.event_id) AS event_count FROM alive_events GROUP BY alive_events.patient_id) AS event_count_alive;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT COUNT(dead_events.event_id) AS event_count FROM dead_events GROUP BY dead_events.patient_id) AS event_count_dead;
...
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(alive_events.time)) AS encounter_count FROM alive_events GROUP BY alive_events.patient_id) AS encounter_count_alive;
...
SELECT avg(encounter_count), percentile(encounter_count, 0.5), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(dead_events.time)) AS encounter_count FROM dead_events GROUP BY dead_events.patient_id) AS encounter_count_dead;
...
-- ***** your code below *****
FROM (SELECT DATEDIFF(TO_DATE(max(alive_events.time)), TO_DATE(min(alive_events.time))) AS record_length FROM alive_events GROUP BY alive_events.patient_id) AS record_length_alive;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT DATEDIFF(TO_DATE(max(dead_events.time)), TO_DATE(min(dead_events.time))) AS record_length FROM dead_events GROUP BY dead_events.patient_id) AS record_length_dead;
